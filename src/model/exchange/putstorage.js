...
class Storage extends Base{
  constructor(data) {
    data = !!data ? data : {}
    super(data)
    this.add_url = `${process.env.API.EXCHANGE}/v2/admin/market/warehousein`
    this.get_list_url = `${process.env.API.EXCHANGE}/v2/admin/market/warehousein`
    this.get_detail_url = `${process.env.API.EXCHANGE}/v2/admin/market/ininfo`
    this.id = data.id || ''
    this.current_model = Storage
    ...
    this.price = parseFloat(data.price || '0')                       //  价格
    ...
    this.list_detail_href = `${process.env.URL.EXCHANGE}/#/market/putstorage/detail?id=${this.bill_sn}`
    this.pkeysearch = {
      title: '',
      type: '',
      bill_sn: '',
    ...
    this.config = {
      add: this.api_rights.market[`v2,market,admin,warehousein,post`],
      detail: this.api_rights.market[`v2,market,admin,ininfo,get`],
      search: true
    }
    this.$storeList = 'MarketPutStorageList'
    this.get_detail_data = data.get_detail_data || {bill_sn: this.bill_sn}
  }

  /**
   * @function
   * @description 设置表单请求发送数据 */
  setPostParam () {
    return {
