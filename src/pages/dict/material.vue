<template>
  <rs-list :page_title="'表壳材质'" :model="model" :column="column">
    <template slot="search">
      ...
      <el-form-item label="表壳材质" prop="name">
        <el-input @keyup.enter.native="model.search()" v-model="model.pkeysearch.name"  placeholder="请输入表壳材质"></el-input>
      </el-form-item>
    </template>
    <template slot-scope="p" slot="form-item">
      <el-form-item label="id" prop="id" v-if="!!p.item && !!p.edit"> {{ p.item.id }} </el-form-item>
  ...
  export default {
    data() {
      return {
        ...
        model: new Goods({'model_type': 'material'}),
        changelang: ''
      }
    },
    mounted() {
