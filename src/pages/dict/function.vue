<template>
  <rs-list :page_title="'复杂功能'" :model="model" :column="column">
    <template slot="search">
      <el-form-item label="商品品类" prop="goods_kind_id" v-if="has_kind_rights">
        <select-scroll
          @change="model.search()"
          :model="kind"
          v-model="model.pkeysearch.goods_kind_id"
          :prop="{label: 'name', value: 'id'}"></select-scroll>
      </el-form-item>
      <el-form-item label="复杂功能" prop="name">
        <el-input @keyup.enter.native="model.search()" v-model="model.pkeysearch.name"  placeholder="请输入名称"></el-input>
      </el-form-item>
    </template>
    <template slot-scope="p" slot="form-item">
      ...
      <el-form-item label="商品品类" prop="goods_kind_id" :rules="{ required: true, message: '请选择商品品类', trigger: 'blur' }" v-if="has_kind_rights">
        <select-scroll :lang="model.web_language"
                       :model="kind"
                       v-model="p.item.goods_kind_id"
                       :prop="{label: 'name', value: 'id'}"></select-scroll>
      </el-form-item>
  ...
  export default {
    data() {
      return {
        ...
        model: new Goods({'model_type': 'functions'}),
        changelang: ''
      }
    },
    mounted() {
      ...
      this.$root.eventHub.$on('add-tab-success', (res) => {
        res.item.model_type = 'functions'
        res.item.setApiUrl()
      })
      /*this.$on('change-lang', (res) => {
        // this.$store.dispatch('getCategoryList', {current: 1, model: Category, lang: res.lang})
        this.changelang = res.lang
      })

      this.$on('change-top-lang', (res) => {
        this.changelang = res.lang
      })*/
    },
   /* methods: {
